...
export function WorkspaceSwitcher(): JSX.Element {
  ...
  return (
    <div className="no-drag relative border-t border-slate-200/70 p-2 dark:border-slate-800">
      {open && (
        <>
          ...
          <div className="absolute bottom-[calc(100%-0.25rem)] left-2 right-2 z-20 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg dark:border-slate-700 dark:bg-slate-800">
            ...
            {workspaces.map((w) =>
              confirmDeleteId === w.id ? (
                ...
                <div
                  key={w.id}
                  className={
                    "group flex items-center pr-1 transition hover:bg-slate-100 dark:hover:bg-slate-700 " +
                    (w.id === activeWorkspaceId ? "text-emerald-700 dark:text-emerald-300" : "text-slate-700 dark:text-slate-200")
                  }
                >
                  <button
                  ...
                  {canDeleteWorkspace && (
                    <button
                      className="shrink-0 rounded p-1 text-slate-400 opacity-0 transition hover:text-rose-500 group-hover:opacity-100"
            ...
            {mode !== "none" ? (
              <div className="px-2 py-2">
                <input
                  autoFocus
                  className="w-full rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[13px] text-slate-700 outline-none focus:border-emerald-300 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
