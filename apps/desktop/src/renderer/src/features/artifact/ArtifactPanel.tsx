...
export function ArtifactPanel(): JSX.Element | null {
  ...
  return (
    <aside className="flex w-[480px] shrink-0 flex-col border-l border-slate-200/70 bg-white/40 dark:border-slate-800 dark:bg-slate-900/30">
...
function BrowserArtifact(): JSX.Element {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="relative shrink-0 border-b border-slate-200/70 dark:border-slate-800">
        <div className="flex items-center gap-1.5 px-3 py-1.5">
          {nav.loading && <Loader2 size={12} className="shrink-0 animate-spin text-emerald-500" />}
          <span className="truncate text-[11.5px] text-slate-500 dark:text-slate-400" title={nav.url}>
            {nav.loading && !nav.url ? "加载中…" : nav.url || "调研浏览器"}
          </span>
        </div>
        {nav.loading && (
          <div className="absolute inset-x-0 bottom-0 h-0.5 animate-pulse bg-emerald-400/70 dark:bg-emerald-500/60" />
        )}
      </div>
      <webview
        ref={ref}
