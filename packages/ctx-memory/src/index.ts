...
- 出现关于用户的、未来仍有用的持久偏好/事实时,用 remember 记下来,并写清 situation(当时在做什么);有关键原话就一并记到 quote。
- **先巩固,再新增**:上文"关于这位用户"段落里每条记忆都带一个 [id]。如果新信息与某条已有记忆**矛盾或细化**(如用户改了归档目录),用 update_memory 改那条(写清 reason),不要堆一条打架的新记忆。
