...
4. **登录态浏览器** 访问闭网内容(Gmail / 内部系统 / 付费订阅等)
   - ~~Playwright + App 内置专用 profile~~ → **改为驱动 Electron 自带 Chromium**(内置 `<webview>` + `persist:research` partition + `webContents.debugger` CDP),**不用 Playwright**。详见下文「内置浏览器调研」节(2026-05-22 定)
...
- **文件整理回滚机制**:执行前整批 **diff 预览 → 审批 → 可逆 journal + 软删除(进回收区,不硬删)**
  ...
  - 明确不用全量复制备份(磁盘爆炸)
- **浏览器**:~~任何改状态动作强制审批~~ → **修订(2026-05-22)**:浏览器操作(click/type)**不弹工具级审批**(可见面板即监督),不可逆对外动作(发送/购买/删除/发帖)改由系统信任边界条款 + guidelines 让模型执行前向用户确认;页面内容防 prompt injection(已落地标注隔离 + 检测)
...
3. ~~**Prompt injection 纵深防御**~~ ✅ **已落地(2026-05-22)**:domain-core 的 `markUntrusted`(分隔符包裹网页内容)+ `detectInjection`(启发式扫注入话术)+ `TRUST_BOUNDARY_PROMPT`(系统提示区分「可信指令」与「网页数据」),疑似命中发 `InjectionSuspected` 事件。详见记忆/代码。
